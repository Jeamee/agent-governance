...
> 定稿 2026-07-15。本文档是跨项目泛化方法论，不绑定任何特定仓库；仓库现状与工具链欠账不写在这里。
...
- 清单在编码前确认冻结；Worker 不得静默删除或缩小范围；冻结后修改必然留痕（清单文件进版本控制，交付 diff 动它即现形）。
...
2. 新增业务决策：每个分支走向必须有显式行为测试（人工/设计期判定，工具测不了）；
...
未经批准，Worker 不得：降低覆盖率阈值、扩大 coverage exclude、新增 `pragma: no cover` / coverage ignore / `skip` / `xfail` / `.only`、删除已有测试变绿、mock 掉本次任务要证明的边界、把 E2E 重试后通过当稳定通过。这些操作不禁止，但必须显式上报为 D 题经人批准。**发现者：CI diff 扫描脚本（必备），过程内 hook 只是提前预警。**

---

## 二、UI/UX Gate（Gate 3 中 FE 行的展开，五层）

UI/UX 没有"覆盖率 100%"这回事。自动化负责证明"行为正确、动效可靠、画面未退化、无障碍降级有效"；人工证据审核负责证明"设计还原、节奏、质感和审美达标"。两者的验收场景都必须编码前冻结，不能由 Worker 临时挑选。
...
3. **视觉回归 gate**：关键已批准状态的稳定截图 diff，防退化不判审美。Playwright `toHaveScreenshot()` 默认已禁动画（有限动画快进终态、无限动画回初态），无需另注 CSS；reduced-motion 是**独立验证场景**，不是截图稳定化手段。**baseline 必须先经人工批准才有资格当 gate**（未批准的截图只是在保护一个可能本来就错的 UI）；截图环境固定（浏览器/OS/viewport/DPR/locale/字体/数据/mask 范围）；优先组件/区域截图，全页只给少量关键流程。
...
录屏由固定 Playwright 场景生成；附正常速度录像、必要时慢速/逐帧、before/after 并排、commit + 命令 + 浏览器版本、手势类交互的真实触屏证据。人工审美按固定 rubric：必要性、频率、响应速度、空间来源、缓动、时长、可打断性、reduced motion、输入方式、性能。

---

## 三、协作流程：设计期对抗审议 + 独立实施 + Proof Gate

> 取消通用的实施后同行代码审查，**不取消独立验证**。设计质量由双 Agent 对抗审议负责；实施 Agent 独立实施；不可由实施者修改的 CI、契约、mutation、真实 E2E 和证据系统负责验收；产品 D 题和审美判断仍由人负责。

### 设计阶段（双 Agent 对抗审议，人做主持人和信道）

**互审不由 agent 自主编排：人分别与两个 agent 对话，把一方的回复转给另一方，逐轮往返；收敛时机和冻结决定权始终在人。** 人做信道的治理价值：每轮论证必然经过人的眼睛，D 题不可能被 agent 之间私下消化，"互相客气提前收敛"和"无限打转"都被主持人压住。

1. **先独立后互审（避免锚定）**：先分别拿到两方的独立第一版——Agent A（产品/前端强项）出业务、用户流程、UI/UX、异常路径；Agent B（架构/后端强项）出架构、数据、状态、并发、测试 oracle、可实施性盲区清单——再开始交叉转发；
2. **每轮回复必须是结构化分歧清单**，不许散文：同意什么 / 反对什么 + 理由 / 新增 D 题 / 撤回的旧立场。方便人搬运、也方便人判断收敛度；
3. **转发保真**：优先文件中转（双方读写同一个 debate 文件），避免人工摘要转述引入失真；人保留"何时触发下一轮"的控制权，不必手搬长文本；
4. **指定一方维护 running 决议账本**（接受 / 拒绝 / D 题 / 理由），每轮更新，防止论证蒸发在对话里；
5. **收敛信号**：连续一轮无新增实质分歧；但这只是信号，收敛判定由人做——**D 题清零或由人裁决后才冻结**。

互审的退出 rubric（人主持时逐项核对）：Known/Unknown 四格是否闭合、状态流转表反向路径是否完备、**每条验收是否机器可自动断言**（设计里写了"人看一眼"的验收，实施阶段没有人）、每条要求是否标注了验证者。

冻结设计包至少包含：业务流程与状态表、架构与跨仓契约、acceptance manifest、UI evidence scenes、架构约束、rollout/rollback/observability、设计版本 hash。**每一项必须标注验证者**（CI / E2E / mutation / static rule / runtime evidence / 人工视觉批准）——没有验证者的设计要求就是未覆盖项。

### 实施阶段（单 Agent 长任务，如 Codex `/goal`）

Goal 不复制整份设计，指向不可变版本：

```text
Outcome: 实现 design_ref=<不可变版本> 的全部要求，完成所有涉及仓库的独立提交。
Constraints: 不得修改冻结设计、gate、coverage、mutation、CI 和 baseline；
  遇到设计外分支暂停并回流设计阶段；不得自行补业务默认值。
...
1. **冻结设计真源在 Worker 写权限之外**；
2. **gate 本身在 Worker 写权限之外**——CI workflow、gate 脚本、coverage/mutation 配置、acceptance schema、screenshot baseline、test runner 配置、设计冻结引用，全部保护；手段：独立 governance repo / 外部 reusable workflow 固定版本 / protected path + CODEOWNERS / 仓库外配置的 required status check；
3. **跨仓组合验证存在**（umbrella integration：设计版本 → 各仓指定 SHA → 契约 + E2E + evidence）；
4. **每条业务与架构要求都有明确验证者**。

**为什么实施者不能有 gate 写权限**：不是能力问题，是审计独立性的结构要求——被验收方不能持有验收规则的写权限。实施 Agent 冲验收时（不需要恶意，"让 CI 变绿"的目标函数使然）会放宽阈值、改脚本、更新 baseline；Gate 5 列的行为全部是 agent 真实会做的事。精确表述：**实施 Agent 可以在设计期贡献治理内容，但"写入冻结仓"的动作必须经人；实施会话不携带治理仓写权限。** 同一工具、不同阶段、不同权限。推荐形态：独立 specs/governance repo 同时充当冻结真源与 gate 治理仓，各项目 CI 以固定版本引用它。

---

## 六、边界诚实条款（机器证明不了的，不假装能证明）

- 审美、节奏、质感：永远靠人按 rubric 审，没有任何机器替代；
